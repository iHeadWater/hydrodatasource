...
def create_test_data_structure(test_data_dir):
    ...
    # 创建目录结构
    dirs_to_create = ["attributes", "timeseries/1D", "shapes", "tggnn"]
...
def create_test_timeseries(test_data_dir):
    ...
    timeseries_file = os.path.join(
        test_data_dir, "timeseries", "1D", "timeseries_1D.nc"
    )
...
def create_test_shapes(test_data_dir):
    ...
    gdf = gpd.GeoDataFrame(
        {"BASIN_ID": basin_names, "geometry": geometries},  # 使用BASIN_ID列名
        crs="EPSG:4326",
    )  # WGS84坐标系
...
def test_tg_hydro_datasource():
    ...
    try:
        ...
        tg_datasource = TgHydroDatasource(
            data_path=test_data_dir, time_unit=["1D"], dataset_name="test_tg_dataset"
        )
        ...
        lstm_predictions = tg_datasource.read_lstm_predictions(
            object_ids=test_basins[:2], t_range_list=["2020-01-01", "2020-01-05"]
        )
        ...
        print(
            f"  时间范围: {lstm_predictions.time.values[0]} 到 {lstm_predictions.time.values[-1]}"
        )
        print(f"✓ 时间序列数据加载成功")
        ...
        node_attrs = tg_datasource.load_node_attributes(
            basin_names=test_basins[:3], selected_attrs=["ele_mt_sav", "area", "p_mean"]
        )
        print(f"✓ 节点属性加载成功")
        ...
        dg, edges, node_mapping = tg_datasource.gen_nx_graph(
            "test_basin", basin_names_all
        )
        print(f"✓ 图生成成功")
        ...
        upstream_nodes = tg_datasource.get_upstream_nodes(
            "test_basin", "station_3", basin_names_all
        )
        print(f"✓ 上游节点查找成功")
