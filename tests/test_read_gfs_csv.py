...
def test_read_gfs_csv():
    ...
    res_lon_list = []
    res_lat_list = []
    for i in range(0, len(gfs_nc_files)):
        test_ds_i = xr.open_dataset(conf.FS.open(gfs_nc_files[i]))
    ...
    test_pd = pd.DataFrame(
        {'bbox': bbox_list, 'time_start': start_time, 'time_end': end_list, 'res_lon': lon_res, 'res_lat': lat_res,
         'path': ['s3://' + file for file in gfs_nc_files]})
    test_pd.to_csv('gfs_metadata.csv')
