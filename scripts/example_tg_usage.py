...
def example_usage():
    ...
    tg_datasource = TgHydroDatasource(
        data_path=data_path, time_unit=["1D"], dataset_name="example_dataset"
    )
    ...
    timeseries_data, lstm_data = tg_datasource.read_timeseries_with_lstm(
        object_ids=selected_basins, t_range=time_range, var_lst=variables
    )
    ...
    attr_data = tg_datasource.read_node_attributes(
        object_ids=selected_basins, selected_attrs=selected_attrs
    )
    ...
    try:
        dg, edges, node_mapping, valid_object_ids = tg_datasource.gen_nx_graph(
            basin_id=basin_id, object_ids=selected_basins
        )
        ...
        if valid_object_ids:
            ...
            upstream_nodes = tg_datasource.get_upstream_nodes(
                basin_id=basin_id, target_node=target_node, object_ids=selected_basins
            )
    ...
    # 读取属性数据
    attr_data_parent = tg_datasource.read_attr_xrdataset(var_lst=["area"])
