...
def calculate_esm(QFi, Qi):#计算平滑度
    ...
    return esm
def get_moving_average_inq():
    inq_data_df = pd.read_csv(os.path.join(definitions.ROOT_DIR, 'example/yingnariver_data/yingna_inq_interpolated_chuli.csv'), engine='c', parse_dates=['TM'])
    ...
    for i in range(2, len(inq_data)-2):
        QF[i] = 1/35*((-3)*inq_data[i-2] + 12*inq_data[i-1] + 17*inq_data[i] + 12*inq_data[i+1] -3*inq_data[i+2])
    QF[len(inq_data)-2] = 1/35*(2*inq_data[len(inq_data)-5] - 8*inq_data[len(inq_data)-4] +12*inq_data[len(inq_data)-3] +27*inq_data[len(inq_data)-2] + 2*inq_data[len(inq_data)-1])
    QF[len(inq_data)-1] = 1/70*((-1)*inq_data[len(inq_data)-5] +4*inq_data[len(inq_data)-4]-6*inq_data[len(inq_data)-3] +4*inq_data[len(inq_data)-2] + 69*inq_data[len(inq_data)-1])
    #卡尔曼滤波
    ...
    inq_bias_df.to_csv(os.path.join(definitions.ROOT_DIR, 'example/yingnariver_data/yingna_bias_inq.csv'))
    bias.to_csv(os.path.join(definitions.ROOT_DIR, 'example/yingnariver_data/yingna_bias.csv'))
    return inq_bias_df
