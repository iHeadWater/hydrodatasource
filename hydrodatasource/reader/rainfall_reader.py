...
LastEditTime: 2025-01-15 11:43:17
    ...
    for root, _, files in os.walk(base_path):
        for file_name in files:
            if file_name.endswith(".csv") and name_filter in file_name:
                ...
                # 读取文件并筛选所需列
                df = pd.read_csv(full_path, usecols=["STCD", "DRP", "TM"], low_memory=False)
...
def filter_out_rows(df1, df2, key_columns):
    ...
    # 过滤掉交集中的行
    filtered_df = df1[~df1[key_columns].apply(tuple, axis=1).isin(common_rows[key_columns].apply(tuple, axis=1))]
...
class RainfallReader(HydroData):
    ...
    def read_station_rainfall(self,abnormal = False):
        ...
        if(abnormal == True):
            abnormal_data =  merge_filtered_data_recursive(RESULT_DIR, ["consistency","extreme","gradient"])
            rainfall_data_list = filter_out_rows(rainfall_data_list, abnormal_data, ["STCD","TM"])
        ...
        rainfall_data = xr.concat(
            [
                ...
                for rainfall, station_id in zip(rainfall_data_list, self.pptn_info["STCD"])
            ],
            dim="STCD",
        )
